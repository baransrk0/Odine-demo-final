...
class RecentMetrics:
    """Keep only length/timing summaries for the most recent terminal turns."""
    ...
    def set_first_audio_started(self, turn_id: UUID, elapsed_ms: float) -> bool:
        """Set observed playback once; repeated reports are idempotent."""
        if elapsed_ms < 0:
            raise ValueError("elapsed_ms must not be negative")
        for entry in reversed(self._entries):
            if entry.turn_id != turn_id:
                continue
            if entry.metrics.first_audio_started_ms is not None:
                return False
            entry.metrics.first_audio_started_ms = elapsed_ms
            return True
        return False
