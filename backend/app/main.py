...
from app.runtimes.protocols import IntentRuntime, LLMRuntime, STTRuntime, TTSRuntime
from app.runtimes.factory import (
...
def create_app(
    ...
    *,
    ...
    stt: STTRuntime | None = None,
    llm: LLMRuntime | None = None,
    tts: TTSRuntime | None = None,
    intent: IntentRuntime | None = None,
    ...
    async def lifespan(application: FastAPI):
        ...
        active_stt = stt or build_stt_runtime(configured_settings)
        active_tts = tts or build_tts_runtime(configured_settings)
        active_llm = llm or LlamaCppClient(configured_settings)
        active_intent = intent
        if active_intent is None:
            active_intent = build_intent_classifier(configured_settings)

        await _load_speech_runtime(active_stt, "STT")
        await _load_speech_runtime(active_tts, "TTS")
        await _probe_runtime(active_llm, "LLM")
        if active_intent is not None:
            # A classifier that never answers is a routing loss, not an outage:
            # the clock rules still fire and everything else falls back.
            await _probe_runtime(active_intent, "Intent")
        ...
        application.state.stt = active_stt
        application.state.tts = active_tts
        application.state.llm = active_llm
        application.state.intent = active_intent
    ...
    async def health(request: Request) -> dict[str, object]:
        ...
        runtimes_ready = {
            "stt_ready": bool(getattr(request.app.state.stt, "ready", False)),
            "tts_ready": bool(getattr(request.app.state.tts, "ready", False)),
            "llm_ready": bool(getattr(request.app.state.llm, "ready", False)),
        }
        # Reported but deliberately left out of `status`: an unreachable
        # classifier costs agent routing, not the turn. The clock rules still
        # fire and every other question falls back to the default agent, so the
        # pipeline can still complete -- which is what `status` claims.
        intent_ready = bool(getattr(request.app.state.intent, "ready", False))
        return {
            "status": "ok" if all(runtimes_ready.values()) else "degraded",
            **runtimes_ready,
    ...
    async def report_playback(
        ...
        await request.app.state.turn_manager.report_playback(
            turn_id,
            report.sequence,
            report.client_offset_ms,
        )
        return Response(status_code=204)

    @application.get("/api/audio/{turn_id}/{sequence}.wav")
    async def audio_chunk(
        request: Request,
        turn_id: UUID,
        sequence: int,
    ) -> Response:
        path = request.app.state.storage.chunk_path(turn_id, sequence)
        if sequence < 0 or not path.is_file():
            raise _audio_not_found()
        try:
            content = await asyncio.to_thread(path.read_bytes)
        except OSError:
            raise _audio_not_found() from None
        return Response(content=content, media_type="audio/wav")

    @application.get("/api/audio/{turn_id}.wav")
    async def complete_audio(
        ...
        if (
            ...
            or turn.metrics.outcome != TurnStatus.COMPLETE
            or not turn.produced_sequences
        ):
            raise _audio_not_found()
        sequences = sorted(turn.produced_sequences)
        if sequences != list(range(sequences[-1] + 1)):
            raise _audio_not_found()
        chunks = [
            request.app.state.storage.chunk_path(turn_id, sequence)
        ...
        try:
            if len(chunks) == 1:
                content = await asyncio.to_thread(chunks[0].read_bytes)
            else:
                content = await asyncio.to_thread(_combine_wav_chunks, chunks)
        except OSError:
            raise _audio_not_found() from None
        return Response(content=content, media_type="audio/wav")
